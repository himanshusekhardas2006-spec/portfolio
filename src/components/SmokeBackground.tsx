...
export function SmokeBackground() {
  ...
  useEffect(() => {
    ...
    // Handle Resize & DevicePixelRatio (capped at 2)
    const resize = () => {
    ...
    const render = () => {
      ...
      if (uSceneLoc) gl.uniform4f(uSceneLoc, canvas.width, canvas.height, seconds * 0.97, 4.0);

      // u_shape = vec4(1.72, 0.60, 0.50, 0.00)
      if (uShapeLoc) gl.uniform4f(uShapeLoc, 1.72, 0.60, 0.50, 0.00);

      // u_surface = vec4(2.40, 1.22, 0.00, 1.00)
      if (uSurfaceLoc) gl.uniform4f(uSurfaceLoc, 2.40, 1.22, 0.00, 1.00);

      // u_finish = vec4(0.00, 0.00, 0.000, 0.00)
      if (uFinishLoc) gl.uniform4f(uFinishLoc, 0.00, 0.00, 0.00, 0.00);

      // u_transform = vec4(635.0, 0.00, 0.00, 0.0)
      if (uTransformLoc) gl.uniform4f(uTransformLoc, 635.0, 0.00, 0.00, 0.0);

      // u_space = vec4(0.00, 0.00, pointer x, pointer y)
      if (uSpaceLoc) gl.uniform4f(uSpaceLoc, 0.00, 0.00, 0.00, 0.00);

      // u_cursor = vec4(presence, 2.0, 0.65, 0.46) -- presence=0.0 when cursor is off
